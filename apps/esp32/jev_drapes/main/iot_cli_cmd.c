...
extern void stepper_test_event(IOT_CAP_HANDLE *handle, int steps, int rpm, int full_steps);
static void _cli_cmd_stepper_test_event(char *string)
{
    ...
    if (_cli_copy_nth_arg(buf, string, sizeof(buf), 2) >= 0) {
        rpm = strtol(buf, NULL, 10);
    }

    if (_cli_copy_nth_arg(buf, string, sizeof(buf), 3) >= 0) {
        full_steps = strtol(buf, NULL, 10);
    }

    printf("stepper_test_event : steps %d\n", steps);
    stepper_test_event(ctx, steps, rpm, full_steps);
}

static struct cli_command cmd_list[] = {
    {"cleanup", "clean-up data with reboot option", _cli_cmd_cleanup},
    {"stepper", "stepper {steps} {rpm} {full_steps}) : ex) stepper_test 2000 300 800", _cli_cmd_stepper_test_event},
};
